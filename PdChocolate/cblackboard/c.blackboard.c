...
void *blackboard_new(t_symbol *s, int argc, t_atom *argv)
{
	...
	x = (t_blacboard *)ebox_alloc(blackboard_class);
    flags = 0
    | EBOX_GROWINDI
    ;
...
void blackboard_clear(t_blacboard *x)
{
    sys_vgui("%s delete %s\n", x->j_box.e_drawing_id->s_name, x->j_box.e_all_id->s_name);
    ebox_redraw((t_ebox *)x);
}
...
void blackboard_line(t_blacboard *x, t_symbol *s, int argc, t_atom *argv)
{
    if(!x->j_box.e_ready_to_draw || x->j_box.e_editor_id == NULL)
        return;
    if(argc > 3 && argv)
    {
        if (atom_gettype(argv) == A_FLOAT && atom_gettype(argv+1) == A_FLOAT && atom_gettype(argv+2) == A_FLOAT && atom_gettype(argv+3) == A_FLOAT)
        {
            sys_vgui("%s create line %d %d %d %d ", x->j_box.e_drawing_id->s_name, (int)atom_getfloat(argv), (int)atom_getfloat(argv+1), (int)atom_getfloat(argv+2), (int)atom_getfloat(argv+3));
            sys_vgui("-fill %s -width %d -tags %s\n", x->f_color->s_name, (int)x->f_width,  x->j_box.e_all_id->s_name);
        }
        ebox_redraw((t_ebox *)x);
    }
}
...
void blackboard_path(t_blacboard *x, t_symbol *s, int argc, t_atom *argv)
{
    ...
    if(argc > 3 && argv)
    {
        ...
        {
            if(x->f_fill)
                sys_vgui("%s create polygon %d %d %d %d ", x->j_box.e_drawing_id->s_name, (int)atom_getfloat(argv), (int)atom_getfloat(argv+1), (int)atom_getfloat(argv+2), (int)atom_getfloat(argv+3));
            else
                sys_vgui("%s create line  %d %d %d %d", x->j_box.e_drawing_id->s_name, (int)atom_getfloat(argv), (int)atom_getfloat(argv+1), (int)atom_getfloat(argv+2), (int)atom_getfloat(argv+3));
            ...
            if(x->f_fill)
                sys_vgui("-fill %s -width 0 -tags %s\n", x->f_color->s_name,  x->j_box.e_all_id->s_name);
            else
                sys_vgui("-fill %s -width %d -tags %s\n", x->f_color->s_name, (int)x->f_width, x->j_box.e_all_id->s_name);
            ebox_redraw((t_ebox *)x);
        }
...
void blackboard_rect(t_blacboard *x, t_symbol *s, int argc, t_atom *argv)
{
    ...
    if(argc > 3 && argv)
    {
        if (atom_gettype(argv) == A_FLOAT && atom_gettype(argv+1) == A_FLOAT && atom_gettype(argv+2) == A_FLOAT && atom_gettype(argv+3) == A_FLOAT)
        {
            sys_vgui("%s create rectangle %d %d %d %d ", x->j_box.e_drawing_id->s_name, (int)atom_getfloat(argv), (int)atom_getfloat(argv+1), (int)atom_getfloat(argv+2), (int)atom_getfloat(argv+3));
...
void blackboard_oval(t_blacboard *x, t_symbol *s, int argc, t_atom *argv)
{
    ...
    if(argc > 3 && argv)
    {
        if (atom_gettype(argv) == A_FLOAT && atom_gettype(argv+1) == A_FLOAT && atom_gettype(argv+2) == A_FLOAT && atom_gettype(argv+3) == A_FLOAT)
        {
            sys_vgui("%s create oval %d %d %d %d ", x->j_box.e_drawing_id->s_name, (int)atom_getfloat(argv), (int)atom_getfloat(argv+1), (int)atom_getfloat(argv+2), (int)atom_getfloat(argv+3));
...
void blackboard_arc(t_blacboard *x, t_symbol *s, int argc, t_atom *argv)
{
    ...
    if(argc > 5 && argv)
    {
        ...
        {
            sys_vgui("%s create arc %d %d %d %d -start %d -extent %d ", x->j_box.e_drawing_id->s_name, (int)atom_getfloat(argv), (int)atom_getfloat(argv+1), (int)atom_getfloat(argv+2), (int)atom_getfloat(argv+3), (int)atom_getfloat(argv+4), (int)atom_getfloat(argv+5));
...
void blackboard_image(t_blacboard *x, t_symbol *s, int argc, t_atom *argv)
{
    ...
    if(argc > 2 && argv)
    {
        ...
        {
            sys_vgui("%s create image %d %d -anchor nw -image [image create photo -file %s/%s] -tags %s\n", x->j_box.e_drawing_id->s_name,  (int)atom_getfloat(argv), (int)atom_getfloat(argv+1), canvas_getdir(x->j_box.e_canvas)->s_name, atom_getsym(argv+2)->s_name, x->j_box.e_all_id->s_name);
            ebox_redraw((t_ebox *)x);
        }
    }
...
void blackboard_text(t_blacboard *x, t_symbol *s, int argc, t_atom *argv)
{
    ...
    if(argc > 2 && argv)
    {
        ...
        {
            sys_vgui("%s create text %d %d -anchor nw -text {", x->j_box.e_drawing_id->s_name, (int)atom_getfloat(argv), (int)atom_getfloat(argv+1));
            for(i = 2; i < argc; i++)
            {
                atom_string(argv+i, buffer, MAXPDSTRING);
                sys_vgui("%s ", buffer);
            }
            sys_vgui("} -font {%s %d %s} -fill %s -tags %s\n", x->j_box.e_font.c_family->s_name, (int)x->j_box.e_font.c_size, x->j_box.e_font.c_weight->s_name, x->f_color->s_name, x->j_box.e_all_id->s_name);
        }
        ebox_redraw((t_ebox *)x);
    }
}
...
void blackboard_pen(t_blacboard *x)
{
    sys_vgui("%s create line %d %d %d %d ", x->j_box.e_drawing_id->s_name, (int)(x->f_pen_old.x), (int)(x->f_pen_old.y), (int)(x->f_pen_new.x), (int)(x->f_pen_new.y));
    sys_vgui("-fill %s -width %d -tags %s\n", x->f_color->s_name, (int)x->f_width,  x->j_box.e_all_id->s_name);
    ebox_redraw((t_ebox *)x);
}
