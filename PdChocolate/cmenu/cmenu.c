...
void cmenu_setup(void)
{
	t_eclass *c;
    
	c = eclass_new("cmenu", (method)menu_new, (method)menu_free, (short)sizeof(t_menu), 0L, A_GIMME, 0);
...
void menu_oksize(t_menu *x, t_rect *newrect)
{
    ...
    x->f_close_height = newrect->height = sys_fontheight(x->j_box.e_font.c_size) + 4;
...
void draw_background(t_menu *x, t_object *view, t_rect *rect)
{
	...
	if(g && jtl)
	{
        ...
        {
            etext_layout_set(jtl, x->f_items[x->f_item_selected]->s_name, &x->j_box.e_font, 1.5, x->f_close_height / 2., rect->width, 0, ETEXT_LEFT, ETEXT_NOWRAP);
        ...
        for(i = 0; i < x->f_items_size; i++)
        {
            ...
            if(x->f_items[i] != gensym("(null)"))
            {
                if(x->f_states[i])
                    ...
                    etext_layout_settextcolor(jtl, &x->f_color_text);
                etext_layout_set(jtl, x->f_items[i]->s_name, &x->j_box.e_font, 1.5, x->f_close_height / 2. + x->f_close_height * (i + 1), rect->width, 0, ETEXT_LEFT, ETEXT_NOWRAP);
                etext_layout_draw(jtl, g);
            }
        }
...
void menu_mousedown(t_menu *x, t_object *patcherview, t_pt pt, long modifiers)
{
    ...
    if(!x->f_hover)
    {
        if(x->f_open)
            ...
            x->f_open = 1;
        atom_setfloat(av, x->j_box.e_rect.width);
