...
void tab_set(t_tab *x, t_symbol *s, int argc, t_atom *argv)
{
    if(argc && argv)
    {
        if(atom_gettype(argv) == A_FLOAT)
            ...
            tab_setsymbol(x, atoms_to_sym(argv, argc));
    }
}
...
void tab_oksize(t_tab *x, t_rect *newrect)
{
    if(x->f_orientation)
    {
        newrect->width = pd_clip_min(newrect->width, sys_fontwidth(x->j_box.e_font.c_size) * 3);
        newrect->height = pd_clip_min(newrect->height, (sys_fontheight(x->j_box.e_font.c_size) + 4) * pd_clip_min(x->f_items_size, 1));
    }
    else
    {
        newrect->width = pd_clip_min(newrect->width, sys_fontwidth(x->j_box.e_font.c_size) * 3 * pd_clip_min(x->f_items_size, 1));
        newrect->height = pd_clip_min(newrect->height, sys_fontheight(x->j_box.e_font.c_size) + 4);
    }
}
        ...
        for(i = 0; i < x->f_items_size; i++)
        {
            if(x->f_items[i] != gensym("(null)"))
            {
                if(x->f_orientation)
                {
                    ...
                    etext_layout_set(jtl, x->f_items[i]->s_name, &x->j_box.e_font, rect->width * 0.5, ratio * (i + 0.5) - sys_fontheight(x->j_box.e_font.c_size) * 0.5, rect->width, 0, ETEXT_UP, ETEXT_JCENTER, ETEXT_NOWRAP);
                    etext_layout_draw(jtl, g);
                }
                else
                {
                    etext_layout_settextcolor(jtl, &x->f_color_text);
                    etext_layout_set(jtl, x->f_items[i]->s_name, &x->j_box.e_font, ratio * (i + 0.5), rect->height * 0.5, ratio - 3, 0, ETEXT_CENTER, ETEXT_JCENTER, ETEXT_WRAP);
                    etext_layout_draw(jtl, g);
                }
            }
        }
...
void tab_mousedown(t_tab *x, t_object *patcherview, t_pt pt, long modifiers)
{
    ...
    if(x->f_orientation)
    {
        index = (pt.y - 2.) / (x->j_box.e_rect.height / (float)x->f_items_size);
    }
    else
    {
        index = (pt.x) / (x->j_box.e_rect.width / (float)x->f_items_size);
    }
