...
extern "C" void cknob_setup(void)
{
	t_eclass *c;
    
	c = eclass_new("cknob", (method)knob_new, (method)knob_free, (short)sizeof(t_knob), 0L, A_GIMME, 0);
...
void knob_mousedown(t_knob *x, t_object *patcherview, t_pt pt, long modifiers)
{
    float angle = pd_angle(pt.x - x->j_box.e_rect.width * 0.5, (x->j_box.e_rect.height * 0.5 - pt.y)) / EPD_2PI;
    if(x->f_mode)
    {
        if(x->f_endless)
        {
