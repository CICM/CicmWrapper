...
extern "C" void cradio_setup(void)
{
	t_eclass *c;
    
	c = eclass_new("cradio", (method)radio_new, (method)radio_free, (short)sizeof(t_radio), 0L, A_GIMME, 0);
...
void *radio_new(t_symbol *s, int argc, t_atom *argv)
{
	...
	binbuf_attr_process(x, d);//To ensure the number of items has been setted before we compute the box size (not clean)
...
void radio_mousedown(t_radio *x, t_object *patcherview, t_pt pt, long modifiers)
{
    ...
    if(x->f_direction)
    {
        index = (pt.x - 2) / x->j_box.e_rect.width * x->f_nitems;
    }
    else
    {
        index = (pt.y - 2) / x->j_box.e_rect.height * x->f_nitems;
    }
