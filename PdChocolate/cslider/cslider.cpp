...
extern "C" void cslider_setup(void)
{
	t_eclass *c;
    
	c = eclass_new("cslider", (method)slider_new, (method)slider_free, (short)sizeof(t_slider), 0L, A_GIMME, 0);
...
void slider_mousedown(t_slider *x, t_object *patcherview, t_pt pt, long modifiers)
{
    ...
    if(x->f_mode)
    {
        ...
        {
            if(x->f_min < x->f_max)
                x->f_value_ref = pd_clip_minmax(pt.x / x->j_box.e_rect.width * ratio + x->f_min, x->f_min, x->f_max);
            else
                x->f_value_ref = pd_clip_minmax((x->j_box.e_rect.width - pt.x) / x->j_box.e_rect.width * ratio + x->f_max, x->f_max, x->f_min);
        }
        else
        {
            if(x->f_min < x->f_max)
                x->f_value_ref = pd_clip_minmax((x->j_box.e_rect.height - pt.y) / x->j_box.e_rect.height * ratio + x->f_min, x->f_min, x->f_max);
            else
                x->f_value_ref = pd_clip_minmax(pt.y / x->j_box.e_rect.height * ratio + x->f_max, x->f_max, x->f_min);
        }
    }
    else
    {
        ...
        {
            if(x->f_min < x->f_max)
                x->f_value = pd_clip_minmax(pt.x / x->j_box.e_rect.width * ratio + x->f_min, x->f_min, x->f_max);
            else
                x->f_value = pd_clip_minmax((x->j_box.e_rect.width - pt.x) / x->j_box.e_rect.width * ratio + x->f_max, x->f_max, x->f_min);
        }
        else
        {
            if(x->f_min < x->f_max)
                x->f_value = pd_clip_minmax((x->j_box.e_rect.height - pt.y) / x->j_box.e_rect.height * ratio + x->f_min, x->f_min, x->f_max);
            else
                x->f_value = pd_clip_minmax(pt.y / x->j_box.e_rect.height * ratio + x->f_max, x->f_max, x->f_min);
        }
        ebox_invalidate_layer((t_ebox *)x, gensym("background_layer"));
        ebox_redraw((t_ebox *)x);
        slider_output(x);
...
void slider_mousedrag(t_slider *x, t_object *patcherview, t_pt pt, long modifiers)
{
    ...
    if(x->f_mode)
    {
        ...
        {
            if(x->f_min < x->f_max)
                newvalue = pt.x / x->j_box.e_rect.width * ratio + x->f_min;
            else
                newvalue = (x->j_box.e_rect.width - pt.x) / x->j_box.e_rect.width * ratio + x->f_max;
        }
        else
        {
            if(x->f_min < x->f_max)
                newvalue = (x->j_box.e_rect.height - pt.y) / x->j_box.e_rect.height * ratio + x->f_min;
            else
                newvalue = pt.y / x->j_box.e_rect.height * ratio + x->f_max;
        }
        if(x->f_min < x->f_max)
