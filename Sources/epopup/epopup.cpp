...
void epopupmenu_additem(t_epopupmenu *menu, int itemid, char *text, t_ergba *textColor, bool checked, bool disabled, void *icon)
{
...
void epopupmenu_popup(t_epopupmenu *menu, t_pt screen, int defitemid)
{
    ...
    t_egraphics* g = ebox_start_layer((t_object *)x, (t_object *)x->e_glist, gensym("cicmpopupmenu"), menu->c_rect.width, menu->c_rect.height);
    
    if(g)
    {
        t_ergba grey = {0.8, 0.8, 0.8, 1.};
        t_ergba greydark = {0.3, 0.3, 0.3, 1.};
        t_ergba black = {0., 0., 0., 1.};
        t_etextlayout *jtl = cicm_text_layout_create();
        cicm_graphics_set_line_width(g, 1.);
        cicm_graphics_set_source_jrgba(g, &grey);
        cicm_graphics_rectangle(g, 0., 0., menu->c_rect.width, menu->c_rect.height);
        ...
        for(int i = 0; i < menu->c_items.size(); i++)
        {
            ...
            {
                cicm_graphics_move_to(g, 0., y);
                cicm_graphics_line_to(g, menu->c_rect.width, y);
                cicm_graphics_fill(g);
            }
            else
            {
                if(!menu->c_items[i].c_disable)
                    cicm_text_layout_settextcolor(jtl, &menu->c_items[i].c_color);
                else
                    cicm_text_layout_settextcolor(jtl, &greydark);
                
                cicm_text_layout_set(jtl, menu->c_items[i].c_text->s_name, &menu->c_font, 7., y, 0., 0., ETEXT_LEFT, ETEXT_NOWRAP);
                cicm_text_layout_draw(jtl, g);
            }
        }
        ebox_end_layer((t_object *)menu->c_obj, (t_object *)x->e_glist, gensym("cicmpopupmenu"));
    }
    ...
    ebox_paint_layer((t_object *)x, (t_object *)x->e_glist, gensym("cicmpopupmenu"), menu->c_rect.x , menu->c_rect.y);
}
        ...
        for(int i = 0; i < menu->c_items.size(); i++)
        {
            ...
            if(pt.x > menu->c_rect.x && pt.x < menu->c_rect.x + menu->c_rect.width && pt.y > y1 + menu->c_rect.y && pt.y < y2  + menu->c_rect.y)
            {
                if(!menu->c_items[i].c_disable && !menu->c_items[i].c_separator)
                {
                    ...
                    t_ergba blue = {0.65, 0.65, 0.65, 1.};
                    t_egraphics* g = ebox_start_layer((t_object *)x, (t_object *)x->e_glist, gensym("cicmpopuplight"), menu->c_rect.width, menu->c_rect.height);
                    t_etextlayout *jtl = cicm_text_layout_create();
                    if(g)
                    {
                        cicm_graphics_set_line_width(g, 1.);
                        cicm_graphics_set_source_jrgba(g, &blue);
                        cicm_graphics_rectangle(g, 1., y1, menu->c_rect.width-1, ratio);
                        cicm_graphics_fill(g);
                        
                        cicm_text_layout_settextcolor(jtl, &menu->c_items[i].c_color);
                        cicm_text_layout_set(jtl, menu->c_items[i].c_text->s_name, &menu->c_font, 7., y2 - 7., 0., 0., ETEXT_LEFT, ETEXT_NOWRAP);
                        cicm_text_layout_draw(jtl, g);
                        
                        ebox_end_layer((t_object *)menu->c_obj, (t_object *)x->e_glist, gensym("cicmpopuplight"));
                    }
                    ebox_paint_layer((t_object *)x, (t_object *)x->e_glist, gensym("cicmpopuplight"), menu->c_rect.x , menu->c_rect.y);
                }
                if(mousedown)
                {
                    ebox_invalidate_layer((t_object *)x, (t_object *)x->e_glist, gensym("cicmpopuplight"));
                    ebox_invalidate_layer((t_object *)x, (t_object *)x->e_glist, gensym("cicmpopupmenu"));
